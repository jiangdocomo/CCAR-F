...
如果业务流程要求 必须执行某个结构化动作，就不要只靠自然语言。
...
The employee handbook, which Claude should read for reference.
create_support_ticket, which creates a new record in the ticketing system.
